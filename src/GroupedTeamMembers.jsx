        ...
        {groupedEmployees.map((item) => {
            ...
            <div key={item.team} className='cursor-pointer mt-7 bg-dark p-5 sm:p-10 w-[75%] sm:w-4/6 rounded-2xl mr-4'>
                ...
                <div className='flex items-center justify-between'>
                  <h4 id={item.team} className='text-white text-center text-sm md:text-xl lg:text-2xl' onClick={handleTeamClick}>
                  Team Name : <span className='text-accent'>{item.team}</span> 
                  </h4>
                  <i className="fa-solid fa-trash text-xs sm:text-2xl lg:text-3xl text-white hover:text-red-500" onClick={() => handleRemoveTeam(item.team)}></i>
                </div>
                ...
                {item.members.map(member => {
                    ...
                    <div key={member.fullName} className='my-8 border-2 border-white p-2 sm:p-8 rounded-lg sm:rounded-3xl'>
                        <h5 className='mb-3  text-white text-sm sm:text-xl'>
      ...
      <div className='flex flex-col pl-16 items-center w-full justify-center mt-6'>
        <div className="mt-6 bg-dark p-2 sm:p-8 mb-6 rounded-2xl py-4 flex flex-col gap-4 items-center justify-center">
          ...
          <button
            ...
            className="ml-4 bg-my-blue text-white text-xs sm:text-md px-2 sm:px-4 py-2 rounded-md">
            Add Team
          </button>
        </div>
      </div>
